...
async function showNumberTrivia(num) {
  const response = await fetch(`${NUMBERS_API_BASE_URL}/${num}?json`);
...
async function showNumberRace(num1, num2, num3, num4) {
  const promise1 = fetch(`${NUMBERS_API_BASE_URL}/${num1}?json`);
  const promise2 = fetch(`${NUMBERS_API_BASE_URL}/${num2}?json`);
  const promise3 = fetch(`${NUMBERS_API_BASE_URL}/${num3}?json`);
  const promise4 = fetch(`${NUMBERS_API_BASE_URL}/${num4}?json`);
  // TODO: Another option, use map to replace lines 24-27 to create a promise array
...
async function showNumberAll(inputs) {
  const promises = inputs.map(
    input => fetch(`${NUMBERS_API_BASE_URL}/${input}?json`));
  ...
  // Use filter to pull out all successful results, then promise.all to get all
  // of the different responses back.
  // TODO: Add another check for a "fullfilled" status on the promiseResult object
  // otherwise the below check wouldn't work if the status was rejected.
